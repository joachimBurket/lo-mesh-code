...
void print_payload(uint8_t *payload, uint16_t size)
{
    printf("payload :\n");
...
void fill_lorawan_pkt_struct(uint8_t *lora_payload, uint16_t size, lorawan_packet_t *pkt)
{
    ...
    print_payload(lora_payload, size);
...
void fill_mesh_data_struct(uint8_t *mesh_payload, uint16_t size, mesh_data_packet_t *pkt)
{
    ...
    memcpy(pkt->payload, &mesh_payload[buff_index], sizeof(pkt->payload)-1);    // todo: is the mesh packet MIC copied in the mesh payload MIC?
