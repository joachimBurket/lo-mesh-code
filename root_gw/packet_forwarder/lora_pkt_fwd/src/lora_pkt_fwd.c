...
void thread_up(void) {
    ...
    // lora mesh header
    uint8_t mesh_header;
    uint8_t mesh_pkt_type;

    // lora mesh packets
    mesh_data_packet_t data_pkt;
    mesh_rreq_packet_t rreq_pkt;
    mesh_rrep_packet_t rrep_pkt;
    mesh_rrer_packet_t rrer_pkt;
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        for (i=0; i < nb_pkt; ++i) {
            ...
            /* Recovering and storing packet fields */
            lorawan_packet_t lorawan_pkt;

            /* Getting the packet informations depending on the packet type */
            fill_lorawan_pkt_struct(p->payload, p->size, &lorawan_pkt);

            switch (lorawan_pkt.mtype) {
                case 1:     // Join accept
                case 3:     // Unconfirmed data down
                case 5:     // Confirmed data down
                case 4:     // Confirmed data up
                    printf("   This packet should not arrive from RF without being encapsulated in lo-mesh.. Skip it\n");
                    continue;   // ignore packet because a join accept from RF must be encapsulated in mesh routing packet
                    break;
                case 0:     // join request
                case 2:     // Unconfirmed data up
                    printf("Packet received from an end device. Will be sent directly to the server\n");
                    break;
                case 7:     // Propretary: a mesh routing packet
                    // Check RFU
                    if(lorawan_pkt.rfu != 1) {
                        printf("   Proprietary packet but not our routing protocol. Skip packet\n");
                        continue;
                    }

                    // Get routing packet type
                    mesh_header = p->payload[1];
                    mesh_pkt_type = (mesh_header & 0xE0) >> 5;
                    printf( "   Routing Packet: mesh_header=0x%08X, mesh_packet_type=0x%08X\n", mesh_header, mesh_pkt_type );

                    switch(mesh_pkt_type) {
                        case DATA_PACKET:
                            fill_mesh_data_struct(&p->payload[2], p->size - 2, &data_pkt);
                            // TODO: Decapsulate the lorawan packet that will be sent to the server
                            memcpy(p->payload, data_pkt.payload, p->size -1);
                            print_payload(p->payload, p->size);
                            break;
                        case RREQ_PACKET:
                            fill_mesh_rreq_struct(&p->payload[2], p->size - 2, &rreq_pkt);
                            // TODO: Send the root address in a RREP response
                            printf("RREQ packet reception not implemented yet\n");
                            break;
                        case RREP_PACKET:
                            fill_mesh_rrep_struct(&p->payload[2], p->size - 2, &rrep_pkt);
                            // TODO: Is it necessary to do something? Root GW will send RREQ packets?
                            printf("RREP packet reception not implemented yet\n");
                            break;
                        case RRER_PACKET:
                            fill_mesh_rrer_struct(&p->payload[2], p->size - 2, &rrer_pkt);
                            // TODO: What to do if RRER packet received?
                            printf("RRER packet reception not implemented yet\n");
                            break;
                        default:
                            printf("   Mesh packet not recognized..\n");
                            continue;
                    }

                    break;
                default:
                    printf("   Unrecognize packet type. Skip packet\n");
                    continue;
            }
    ...
    while (!exit_sig && !quit_sig) {
        ...
        while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
            ...
            if (jit_result == JIT_ERROR_OK) {
                ...
                lorawan_packet_t pkt;
                fill_lorawan_pkt_struct(txpkt.payload, txpkt.size, &pkt);

                // check packet type
                switch (pkt.mtype) {
                    case 1:     // Join accept
                        // TODO send the packet to a potential nearby end device + broadcast to all mesh GW

                        break;
                    case 3:     // Unconfirmed data down
                    case 5:     // Confirmed data down
                        // TODO: send the packet to the good GW if for a GW, or to a nearby end device (depending on the device table)
                        printf("Data down not implemented yet\n");
                        break;
                    case 0:     // join request
                    case 2:     // Unconfirmed data up
                    case 4:     // Confirmed data up
                    case 7:
                        printf("This packet should not arrive from the server\n");
                        //continue;
                        break;
                    default:
                        printf("Hmm.. what a strange lorawan packet type\n");
                }
                printf("================================================================================\n");
                //================================================================================================
