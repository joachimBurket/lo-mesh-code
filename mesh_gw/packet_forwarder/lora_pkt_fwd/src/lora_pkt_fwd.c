...
int main(void)
{
    ...
    /* initialize lo-mesh gateway */
    routing_table_entry_t root_entry = {
            .dst_node_id = 10,      // TODO: B827EBFFFEC3D4B3
            .next_hop_id = 11,      // TODO: B827EBFFFEC3D4B3
            .hop_count = 1,     // 0 or 1 ?
            .dst_seq_no = 0,
            .valid = true,
    };
    init_mesh_gw(&root_entry, &lgwm);

    /* process some of the configuration variables */
    net_mac_h = htonl((uint32_t)(0xFFFFFFFF & (lgwm>>32)));
    net_mac_l = htonl((uint32_t)(0xFFFFFFFF &  lgwm  ));

#define LO_MESH 1   // define to comment moultes things not used in the mesh gateway
#ifndef LO_MESH
...
#ifndef LO_MESH    // Don't create donwlink thread on mesh gw
...
void thread_up(void) {
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        /* fetch packets */
        pthread_mutex_lock(&mx_concent);
        //printf("DEBUG: Getting lora gw packets\n");
        ...
        if ((nb_pkt == 0) && (send_report == false)) {
            //printf("DEBUG: No new packet nor report; wait and retry\n");
        ...
        pkt_in_dgram = 0;
        for (i=0; i < nb_pkt; ++i) {
            printf("\nDEBUG: Start reading %d packets\n", nb_pkt);

            p = &rxpkt[i];


            //========================================= LO-MESH CODE =========================================
            printf("================================= LO-MESH UP LOGS =================================\n");
            /* Base Payload Received */
            uint8_t *payload = p->payload;
            uint8_t payloadSize = p->size;

            /* Mac Header*/
            LoRaMacHeader_t macHdr;

            /* Messages structures */
            //LoRaMacMessage_t macMsg;      // TODO: Only one message instead of one of each type?
            //LoRaMeshMessage_t meshMsg;
            LoRaMacMessageJoinRequest_t macMsgJoinRequest;
            LoRaMacMessageData_t macMsgData;
            LoRaMeshMessageRouteRequest_t meshMsgRouteRequest;
            LoRaMeshMessageRouteResponse_t meshMsgRouteResponse;
            LoRaMeshMessageRouteError_t meshMsgRouteError;
            LoRaMeshMessageData_t meshMsgData;


            /* Status variables */
            LoRaMacParserStatus_t macParserStatus = LORAMAC_PARSER_ERROR;
            LoRaMeshParserStatus_t meshParserStatus = LORAMESH_PARSER_ERROR;
            ...
            switch (macHdr.Bits.MType) {
                case FRAME_TYPE_JOIN_ACCEPT:                // Join accept
                    printf("   This packet should not arrive from RF without being encapsulated in lo-mesh packet..\n");
                case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:      // Unconfirmed data down
                case FRAME_TYPE_DATA_CONFIRMED_DOWN:        // Confirmed data down
                case FRAME_TYPE_DATA_CONFIRMED_UP:          // Confirmed data up
                    printf("   Skip this packet (ignore confirmed data packets or join_accept)\n");
                    continue;
                    break;
                case FRAME_TYPE_JOIN_REQ:                   // join request
                    printf("   Join request packet\n");
                    /* Parse packet in structure */
                    macMsgJoinRequest.Buffer = payload;
                    macMsgJoinRequest.BufSize = payloadSize;

                    macParserStatus = LoRaMacParserJoinRequest( &macMsgJoinRequest );
                    if( macParserStatus != LORAMAC_PARSER_SUCCESS ) {
                        printf("   Error while parsing MAC Join Request message with code %d\n", macParserStatus);
                        continue;
                    }
                    // Print parsed struct
                    LoRaMacJoinRequestToString(&macMsgJoinRequest);

                    /* TODO: Encapsulate packet */


                    /* TODO: Send to next hop */

                    break;
                case FRAME_TYPE_DATA_UNCONFIRMED_UP:        // Unconfirmed data up
                    printf("   Unconfirmed data up packet\n");
                    /* Parse packet in structure */
                    macMsgData.Buffer = payload;
                    macMsgData.BufSize = payloadSize;

                    //macParserStatus = LoRaMacParserData( &macMsgData );   // FIXME
                    if( macParserStatus != LORAMAC_PARSER_SUCCESS ) {
                        printf("   Error while parsing MAC Data message with code %d\n", macParserStatus);
                        continue;
                    }
                    // Print parsed struct
                    LoRaMacDataToString(&macMsgData);

                    /* TODO: Encapsulate packet */
                    /* TODO: Send to next hop */
                    break;
                case 7:                                     // Propretary: a mesh routing packet
                    printf("   Proprietary packet with RFU=%d\n", macHdr.Bits.RFU);
                    // Check RFU
                    if(macHdr.Bits.RFU != 1) {
                        printf("   Proprietary packet but not our routing protocol. Skip packet\n");
                        continue;
                    }

                    // Get routing packet type
                    LoRaMeshHeader_t meshHdr;
                    meshHdr.Value = payload[1];
                    printf( "   Routing Packet: mesh_header=0x%08X, mesh_packet_type=0x%08X\n", meshHdr.Value, meshHdr.Bits.MType );

                    switch(meshHdr.Bits.MType) {
                        case LORAMESH_MSG_TYPE_DATA:
                            meshMsgData.Buffer = payload;
                            meshMsgData.BufSize = payloadSize;
                            meshParserStatus = LoRaMeshParserData( &meshMsgData );
                            if( meshParserStatus != LORAMESH_PARSER_SUCCESS ) {
                                printf("   Error while parsing Mesh Data message with code %d\n", meshParserStatus);
                                continue;
                            }
                            // Print parsed struct
                            LoRaMeshDataToString(&meshMsgData);

                            /* TODO into a function ! */
                            switch(meshMsgData.EncapLoRaMacMessage.Type) {
                                case LORAMAC_MSG_TYPE_JOIN_REQUEST:
                                    printf("   Encapsulated Join Request. Behavior not implemented yet\n");
                                    // TODO: Code behavior
                                    break;
                                case LORAMAC_MSG_TYPE_JOIN_ACCEPT:
                                    printf("   Encapsulated Join Accept. Behavior not implemented yet\n");
                                    // TODO: Code behavior
                                    break;
                                case LORAMAC_MSG_TYPE_DATA:
                                    printf("   Encapsulated Data. Behavior not implemented yet\n");
                                    // TODO: Code behavior
                                    break;
                                default:
                                    printf("   Error unrecognized mac packet type (%d)\n", meshMsgData.EncapLoRaMacMessage.Type);
                            }

                            break;
                        case LORAMESH_MSG_TYPE_ROUTE_REQ:
                            meshMsgRouteRequest.Buffer = payload;
                            meshMsgRouteRequest.BufSize = payloadSize;
                            meshParserStatus = LoRaMeshParserRouteRequest( &meshMsgRouteRequest );
                            if( meshParserStatus != LORAMESH_PARSER_SUCCESS ) {
                                printf("   Error while parsing Mesh Route Request message with code %d\n", meshParserStatus);
                                continue;
                            }
                            // Print parsed struct
                            LoRaMeshRouteRequestToString(&meshMsgRouteRequest);
                            printf("   Route Request Message. Behavior not implemented yet\n");
                            // TODO: Code behavior
                            break;
                        case LORAMESH_MSG_TYPE_ROUTE_RESP:
                            meshMsgRouteResponse.Buffer = payload;
                            meshMsgRouteResponse.BufSize = payloadSize;
                            meshParserStatus = LoRaMeshParserRouteResponse( &meshMsgRouteResponse );
                            if( meshParserStatus != LORAMESH_PARSER_SUCCESS ) {
                                printf("   Error while parsing Mesh Route Response message with code %d\n", meshParserStatus);
                                continue;
                            }
                            // Print parsed struct
                            LoRaMeshRouteResponseToString(&meshMsgRouteResponse);
                            printf("   Route Response Message. Behavior not implemented yet\n");
                            // TODO: Code behavior
                            break;
                        case LORAMESH_MSG_TYPE_ROUTE_ERR:
                            meshMsgRouteError.Buffer = payload;
                            meshMsgRouteError.BufSize = payloadSize;
                            meshParserStatus = LoRaMeshParserRouteError( &meshMsgRouteError );
                            if( meshParserStatus != LORAMESH_PARSER_SUCCESS ) {
                                printf("   Error while parsing Mesh Route Error message with code %d\n", meshParserStatus);
                                continue;
                            }
                            // Print parsed struct
                            LoRaMeshRouteErrorToString(&meshMsgRouteError);
                            printf("   Route Error Message. Behavior not implemented yet\n");
                            // TODO: Code behavior
                            break;
                        default:
                            printf("   Unrecognized mesh packet type (%d). Skip packet\n", meshHdr.Bits.MType);
                            continue;
                    }
                    break;
                default:
                    printf("   Unrecognized mac packet type (%d). Skip packet\n", macHdr.Bits.MType);
                    continue;
            }
...
void thread_down(void) {
    ...
    *(uint32_t *)(buff_req + 8) = net_mac_l;
    ...
    jit_queue_init(&jit_queue);     // TODO: In the main() ?
    ...
    while (!exit_sig && !quit_sig) {
        ...
        while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
            ...
            if (jit_result == JIT_ERROR_OK) {
                ...
                //========================================= LO-MESH CODE =========================================
                printf("================================= LO-MESH DOWN LOGS =================================\n");
                // TODO: DEBUG
                printf("DEBUG: ============ downlink packet to be sent to the device ===========\n");
                lorawan_packet_t debug_pkt;
                fill_lorawan_pkt_struct(txpkt.payload, txpkt.size, &debug_pkt);
                printf("================================================================================\n");
                //================================================================================================
